...
class ExampleHouse(Map):
    ...
    def move(self, player: HumanPlayer, direction_s: str) -> list[Message]:
        ...
        Preconditions:
            - player is not None and has get_current_position()
            - direction_s is in ['up', 'down', 'left', 'right']
            - garden_grid is initialized
        ...
        assert hasattr(player, 'get_current_position'), "Player must track position"
        assert direction_s in ['up', 'down', 'left', 'right'], "Invalid direction"
        assert hasattr(self, 'garden_grid'), "Garden grid not initialized"
        assert hasattr(self.garden_grid, 'get_grid_origin'), "Grid missing origin method"
    ...
    def update_player_in_garden(self,player:HumanPlayer) -> list[Message]:
        """
        Updates player with proper messages based on the player's position

        Preconditions:
            - player is not None and has get_current_position().
            - garden grid is initialized 
        ...
        assert player is not None, "Player cannot be None"
        assert hasattr(player, 'get_current_position'), "Player must track position"
        assert hasattr(self.garden_grid, 'player_entered'), "Grid missing entered logic"
        assert hasattr(self.garden_grid, 'player_exited'), "Grid missing exited logic"
